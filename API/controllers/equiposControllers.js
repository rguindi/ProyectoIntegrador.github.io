...
const getEquipoById = (req, res) => {
  //http://localhost:3000/equipo/registro/3
...
const crearEquipo = (req, res) => {
  ...
  db.query(
    "INSERT INTO equiposelectronicos (nombre, descripcion, marca, modelo, numero_de_serie, estado, id_aula, id_categoria, imagen_producto, qr_code, ano_adquisicion, ultima_actualizacion, codigo) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
    [
      nombre,
      descripcion,
      marca,
...
const putEquipo = (req, res) => {
  ...
  const qr_code = req.files[1].originalname;
  const sql =
    "UPDATE equiposelectronicos SET nombre=?, descripcion=?, marca=?, modelo=?, numero_de_serie=?, estado=?, id_aula=?, id_categoria=?, imagen_producto=?, qr_code=?, ano_adquisicion=?, ultima_actualizacion=?, codigo=? WHERE id_equipo = ?";
  db.query(
    sql,
    [
      nombre,
